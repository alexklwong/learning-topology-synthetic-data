...
class FeaturePyramidDecoder(torch.nn.Module):
    '''
    ...
    Arg(s):
        ...
        target_resolution : int
            target resolution 1/2^N for pyramid, the rest will be upsampled with convolutions
        n_filters : list[int]
            number of filters to use at each decoder block
        n_skips : list[int]
            number of filters from skip connections
    ...
    def __init__(self,
                 ...
                 use_instance_norm=False,
                 deconv_type='transpose'):
        super(FeaturePyramidDecoder, self).__init__()

        network_depth = len(n_filters)

        assert network_depth < 8, 'Does not support network depth of 8 or more'
        assert target_resolution > 0 and target_resolution < network_depth
        ...
        self.deconv_latent = net_utils.DecoderBlock(
            input_channels,
            0,
            n_filters[0],
            weight_initializer=weight_initializer,
        ...
        pyramid_channels = pyramid_channels + n_filters[0]

        skip_channels, out_channels = [
            n_skips[filter_idx], n_filters[filter_idx]
        ]
        ...
        if network_depth > 6:
            ...
            pyramid_channels = pyramid_channels + out_channels
        ...
        for dilation in [2, 4, 8]:
            atrous_pyramid1.append(
                net_utils.AtrousConv2d(
                    out_channels,
                    out_channels,
                    kernel_size=3,
                    dilation=dilation,
        ...
        self.atrous_pyramid1 = torch.nn.Sequential(*atrous_pyramid1)
    ...
    def forward(self, x, skips, shape=None):
        ...
        # TODO: Extend to general pyramid, currently supports 1/8 (1/2^3) pyramid
        assert self.target_resolution == 2

        target_resolution = self.target_resolution - 1
        shape_pyramid = skips[target_resolution].shape[-2:]

        layers = [self.deconv_latent(x, skip=None, shape=shape_pyramid)]
        outputs = []
        ...
        # Resolution 1/128 -> 1/64
        if self.deconv6 is not None:
            layers.append(self.deconv6(skips[n], skip=None, shape=shape_pyramid))
            n = n - 1

        # Resolution 1/64 -> 1/32
        if self.deconv5 is not None:
            layers.append(self.deconv5(skips[n], skip=None, shape=shape_pyramid))
            n = n - 1

        # Resolution 1/32 -> 1/16
        if self.deconv4 is not None:
            layers.append(self.deconv4(skips[n], skip=None, shape=shape_pyramid))
            n = n - 1

        # Resolution 1/16 -> 1/8
        if self.deconv3 is not None:
            layers.append(self.deconv3(skips[n], skip=None, shape=shape_pyramid))
            n = n - 1

        # Resolution 1/8 -> 1/4
        pyramid = torch.cat(layers, dim=1)
        layers.append(self.deconv2(pyramid, skip=skips[n]))
        n = n - 1

        # Resolution 1/4 -> 1/2
        layers.append(self.deconv1(layers[-1], skips[n]))
        layers.append(self.atrous_pyramid1(layers[-1]))
