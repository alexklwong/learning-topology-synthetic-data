...
class FusionNetModel(object):
    ...
    def __init__(self,
        ...
        if 'vggnet08' in encoder_type:
            ...
            self.encoder_image = networks.ResNetEncoder(
                n_layer=18,
                input_channels=3,
                n_filters=n_filters_encoder_image,
                weight_initializer=weight_initializer,
                ...
                use_batch_norm='batch_norm' in encoder_type,
                use_instance_norm='instance_norm' in encoder_type)
            self.encoder_depth = networks.ResNetEncoder(
                n_layer=18,
                input_channels=2,
                n_filters=n_filters_encoder_depth,
                weight_initializer=weight_initializer,
        ...
        if 'multi-scale' in decoder_type:
            ...
            self.decoder = networks.FeaturePyramidDecoder(
                input_channels=n_filters_encoder[-1],
                output_channels=2,
                target_resolution=2,
                n_filters=n_filters_decoder,
    ...
    def compute_loss_supervised(self,
                                output_depth,
                                ground_truth,
                                w_supervised_l1=1.00,
                                w_supervised_l2=1.00):
        '''
        Computes loss function
        l = w_{l1}l_{l1} + w_{l2}l_{l2}

        Arg(s):
            image : torch.Tensor[float32]
                N x 3 x H x W input image
            output_depth : torch.Tensor[float32]
                N x 1 x H x W output depth
            ground_truth : torch.Tensor[float32]
                N x 1 x H x W ground truth
            w_supervised_l1 : float
                weight of supervised l1 loss
            w_supervised_l2 : float
                weight of supervised l2 loss
            w_supervised_temporal : float
                weight of supervised tempora loss
            edge_weight_kernel_size : int
                symmetric kernel size to produce edge weight map
        '''
        ...
        if w_supervised_l1 > 0.00:
            loss_l1 = \
                losses.l1_loss_func(output_depth, ground_truth, w=validity_map)
        else:
            loss_l1 = 0.00

        if w_supervised_l2 > 0.00:
            loss_l2 = \
                losses.l2_loss_func(output_depth, ground_truth, w=validity_map)
        else:
            loss_l2 = 0.00

        loss = w_supervised_l1 * loss_l1 + w_supervised_l2 * loss_l2
    ...
    def compute_loss_unsupervised(self,
                                  output_depth0,
                                  sparse_depth0,
                                  validity_map0,
                                  input_depth0,
        ...
        sparse_depth0 = torch.where(
            ...
            torch.full_like(sparse_depth0, self.max_predict_depth),
            sparse_depth0)
        ...
        if w_prior_depth > 0.0 and loss_color1to0 < threshold_prior_depth:
            ...
            points_prior = loss_utils.backproject_to_camera(input_depth0, intrinsics, shape)
            target_xy0to1_prior = loss_utils.project_to_pixel(points_prior, pose0to1, intrinsics, shape)

            # Reconstruct image0 from image1 and image2 by reprojection
            image1to0_prior = loss_utils.grid_sample(image1, target_xy0to1_prior, shape)
    ...
    def save_model(self, checkpoint_path, step, optimizer, scaffnet_model=None):
        ...
        if scaffnet_model is not None:
            # Save ScaffNet encoder weights
            if isinstance(scaffnet_model.encoder, torch.nn.DataParallel):
                checkpoint['scaffnet_encoder_state_dict'] = \
                    scaffnet_model.encoder.module.state_dict()
            else:
                checkpoint['scaffnet_encoder_state_dict'] = scaffnet_model.encoder.state_dict()

            # Save ScaffNet depth decoder weights
            if isinstance(scaffnet_model.decoder_depth, torch.nn.DataParallel):
                checkpoint['scaffnet_decoder_depth_state_dict'] = \
                    scaffnet_model.decoder_depth.module.state_dict()
            else:
            ...
            if 'spatial_pyramid_pool' in scaffnet_model.encoder_type:
                ...
                if isinstance(scaffnet_model.spatial_pyramid_pool, torch.nn.DataParallel):
                    checkpoint['spatial_pyramid_pool_state_dict'] = \
                        scaffnet_model.spatial_pyramid_pool.module.state_dict()
                else:
                    checkpoint['spatial_pyramid_pool_state_dict'] = \
                        scaffnet_model.spatial_pyramid_pool.state_dict()

            if 'uncertainty' in scaffnet_model.decoder_type:
                # Save ScaffNet uncertainty decoder weights
                if isinstance(scaffnet_model.decoder_uncertainty, torch.nn.DataParallel):
                    checkpoint['scaffnet_decoder_uncertainty_state_dict'] = \
                        scaffnet_model.decoder_uncertainty.module.state_dict()
                else:
                    checkpoint['scaffnet_decoder_uncertainty_state_dict'] = \
                        scaffnet_model.decoder_uncertainty.state_dict()

        # Save training state
        checkpoint['train_step'] = step
        checkpoint['fusionnet_optimizer_state_dict'] = optimizer.state_dict()

        # Save FusionNet encoder and decoder weights
        if isinstance(self.encoder_image, torch.nn.DataParallel):
            checkpoint['fusionnet_encoder_image_state_dict'] = self.encoder_image.module.state_dict()
        else:
            checkpoint['fusionnet_encoder_image_state_dict'] = self.encoder_image.state_dict()

        if isinstance(self.encoder_image, torch.nn.DataParallel):
            checkpoint['fusionnet_encoder_depth_state_dict'] = self.encoder_depth.module.state_dict()
        else:
            checkpoint['fusionnet_encoder_depth_state_dict'] = self.encoder_depth.state_dict()

        if isinstance(self.encoder_image, torch.nn.DataParallel):
            checkpoint['fusionnet_decoder_state_dict'] = self.decoder.module.state_dict()
        else:
    ...
    def restore_model(self, checkpoint_path, optimizer=None, scaffnet_model=None):
        ...
        if scaffnet_model is not None:
            # Restore ScaffNet encoder weights
            if isinstance(scaffnet_model.encoder, torch.nn.DataParallel):
                scaffnet_model.encoder.module.load_state_dict(checkpoint['scaffnet_encoder_state_dict'])
            else:
                scaffnet_model.encoder.load_state_dict(checkpoint['scaffnet_encoder_state_dict'])

            # Restore ScaffNet depth decoder weights
            if isinstance(scaffnet_model.decoder_depth, torch.nn.DataParallel):
                scaffnet_model.decoder_depth.module.load_state_dict(checkpoint['scaffnet_decoder_depth_state_dict'])
            else:
            ...
            if 'spatial_pyramid_pool' in scaffnet_model.encoder_type:
                if 'spatial_pyramid_pool_state_dict' in keys:
                    ...
                    if isinstance(scaffnet_model.spatial_pyramid_pool, torch.nn.DataParallel):
                        scaffnet_model.spatial_pyramid_pool.module.load_state_dict(
                            checkpoint['spatial_pyramid_pool_state_dict'])
                    else:
                        scaffnet_model.spatial_pyramid_pool.load_state_dict(
                            checkpoint['spatial_pyramid_pool_state_dict'])

            # Restore ScaffNet uncertainty decoder weights
            if 'uncertainty' in scaffnet_model.decoder_type:
                if 'scaffnet_decoder_uncertainty_state_dict' in keys:

                    if isinstance(scaffnet_model.decoder_uncertainty, torch.nn.DataParallel):
                        scaffnet_model.decoder_uncertainty.module.load_state_dict(
                            checkpoint['scaffnet_decoder_uncertainty_state_dict'])
                    else:
                        scaffnet_model.decoder_uncertainty.load_state_dict(
                            checkpoint['scaffnet_decoder_uncertainty_state_dict'])

        # Restore FusionNet encoder and decoder weights
        if isinstance(self.encoder_image, torch.nn.DataParallel):
            self.encoder_image.module.load_state_dict(checkpoint['fusionnet_encoder_image_state_dict'])
        else:
        ...
        if isinstance(self.encoder_depth, torch.nn.DataParallel):
            self.encoder_depth.module.load_state_dict(checkpoint['fusionnet_encoder_depth_state_dict'])
        else:
        ...
        if isinstance(self.decoder, torch.nn.DataParallel):
            self.decoder.module.load_state_dict(checkpoint['fusionnet_decoder_state_dict'])
        else:
        ...
        with torch.no_grad():
            ...
            for image_disp, image_disp_txt in zip(image_list, image_text_list):
                ...
                if image0 is not None and image_disp is not None:
                    image_disp_summary = image_disp[0:n_image_per_summary, ...]
            ...
            if output_depth0 is not None and input_depth0 is not None:
                input_depth0_summary = input_depth0[0:n_image_per_summary, ...]
