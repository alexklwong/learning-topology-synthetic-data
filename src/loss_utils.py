...
# and code written by Alex Wong and Xiaohan Fei:
...
def pose_vec2mat(vec):
    ...
    Returns:
        tensor : a transformation matrix -- [B, 4, 4]
    '''
...
def pose_expm(vec):
    ...
    Args:
        vec : tensor
            6DoF parameters in the order of tx, ty, tz, rx, ry, rz -- [B, 6]
    Returns:
        tensor : homogneneous matrix of the transformation -- [B, 4, 4]
    '''
...
def pixel2cam(depth, pixel_coords, intrinsics, is_homogeneous=True):
    ...
    Args:
        ...
        pixel_coords :
            homogeneous pixel coordinates [batch, 3, height, width]
        intrinsics : camera intrinsics
            [batch, 3, 3] intrinsics matrix
        is_homogeneous : return in homogeneous coordinates
    Returns:
        tensor : coords in the camera frame [batch, 3 (4 if homogeneous), height, width]
    '''
...
def cam2pixel(cam_coords, proj):
    ...
    Args:
        cam_coords : tensor
            [batch, 4, height, width] tensor
        proj : tensor
            [batch, 4, 4] matrices
    Returns:
        tensor : pixel coordinates projected from the camera frame [batch, height, width, 2]
    '''
...
def meshgrid(batch, height, width, is_homogeneous=True):
    ...
    Args:
        ...
        width : int
             width of the grid
        is_homogeneous : bool
            whether to return in homogeneous coordinates
    Returns:
        tensor : x,y grid coordinates [batch, 2 (3 if homogeneous), height, width]
    '''
...
def flow_warp(src_img, flow):
    ...
    Args:
        src_img : tensor
            the source  image [batch, height_s, width_s, 3]
        flow : tensor
            target image to source image flow [batch, height_t, width_t, 2]
    Returns:
        tensor :
...
def compute_rigid_flow(depth, pose, intrinsics, reverse_pose=False):
    ...
    Args:
        depth : tensor
            depth map of the target image [batch, height_t, width_t]
        pose : tensor
            target to source (or source to target if reverse_pose=True)
            camera transformation matrix [batch, 6], in the order of
            tx, ty, tz, rx, ry, rz;
        intrinsics: camera intrinsics [batch, 3, 3]
    Returns:
        tensor : rigid flow from target image to source image [batch, height_t, width_t, 2]
    '''
...
def bilinear_sampler(imgs, coords):
    ...
    Args:
        imgs : tensor
            source image to be sampled from [batch, height_s, width_s, channels]
        coords : tensor
            coordinates of source pixels to sample from [batch, height_t,
            width_t, 2]. height_t/width_t correspond to the dimensions of the output
            image (don't need to be the same as height_s/width_s). The two channels
            correspond to x and y coordinates respectively.
    Returns:
        tensor : a new sampled image [batch, height_t, width_t, channels]
    '''
...
def inverse_pose(g):
    ...
    Args
        g : tensor
            Bx4x4 pose batch.
    Returns:
        tensor : B x 4 x 4 inverse pose g_inv
    '''
    ...
    def _inverse_pose(g):
        ...
        g_inv = tf.concat((Rt, -tf.matmul(Rt, tf.expand_dims(g[:3, 3], axis=1))), axis=1)
        g_inv = tf.concat((g_inv, [[0, 0, 0, 1]]), axis=0)
